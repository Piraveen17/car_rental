...
import {
  Car, LayoutDashboard, Calendar, Users, Wrench, BarChart3,
  LogOut, Menu, X, ChevronRight, ExternalLink, Shield
} from "lucide-react"
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (isHydrated && isAuthenticated && user?.role !== "admin") {
      router.replace(user?.role === "staff" ? "/staff" : "/dashboard")
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 h-16 border-b shrink-0">
          ...
          >
            ...
            <div className="leading-none">
              ...
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-0.5">Admin</p>
            </div>
          </Link>
          <Button
            variant="ghost"
        ...
        <div className="p-3 border-t space-y-2 shrink-0">
          <div className="flex items-center gap-2 px-2 py-2 rounded-lg bg-muted/50">
            ...
            <Badge variant="secondary" className="text-[9px] px-1 py-0 shrink-0">Admin</Badge>
          </div>
