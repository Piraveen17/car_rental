...
export async function updateSession(request: NextRequest) {
  ...
  if (user) {
    // We'll need to fetch the role from metadata or a table
    // For now, let's assume metadata contains the role as per plan
    ...
    if (staffRoutes.some(path => request.nextUrl.pathname.startsWith(path)) && role !== 'admin' && role !== 'staff') {
       return NextResponse.redirect(new URL('/', request.url))
    }
  }
